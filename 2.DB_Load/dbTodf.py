...
def newsdb2df(result)->bool:
    ...
    try:
        ...
        port=3306,
        user='root',
        passwd=open(r'C:\Users\Choi\workspace\sesac\project1_crawring\yycrawler_package\MK_news_pkg\config_files\mysql_config','r').read().strip(),
        db='newsdb',
        charset='utf8')
        ...
        #db조회 최신순으로 가져오기 위해 id 기준으로 desc정렬 후 110개 가져오기
        #데이터 프레임으로 가져오기
        try:
            import pandas as pd
            ...
            news_top10_temp=pd.read_sql('SELECT * FROM MK_best ORDER BY id DESC LIMIT 110',connect)
